  ...
  return (
    <ul className="flex gap-10 justify-start items-center">
      {HomePageNav.map((navData) => (
        <li
          className="text-slate-200 -600/90 drop-shadow-lg text-lg font-semibold"
          key={navData.id}
        >
          <NavLink 
...
function Header() {
  ...
  return (
    <nav className="fixed opacity-[90%] z-10 w-[60vw] mx-[20vw] rounded-lg top-3 h-[60px] flex justify-between p-3 items-center bg-slate-950/80  -200 border-slate-800 border-2 shadow-lg">
      <div className="w-[20%] ">
       <h1 className="text-3xl text-white">Localify</h1>
      </div>
      ...
      <div className="h-full flex items-center relative overflow-visible">
        {!status && (
          <button
            className="w-[70px] text-lg font-semibold text-white hover:scale-95 rounded-xl h-[40px]"
        ...
        {!status && (
          <button
            className="w-[75px] text-lg font-semibold text-white hover:scale-95  rounded-xl h-[40px] bg-pink-900"
