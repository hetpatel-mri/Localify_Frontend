...
function Shops() {
  ...
  return (
    <div className="min-h-screen GradientNeon p-20 ">
      <header className="bg-slate-950 border-2 border-purple-700 my-4   shadow-md  rounded-lg">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-200">Our Partners</h1>
          <div className="flex items-center space-x-4">
            <div className="relative">
              <Input
                type="search"
                placeholder="Search shops..."
                className="pl-10 pr-4 py-2 bg-black text-white rounded-full"
                value={searchTerm}
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-3xl font-bold text-gray-300 overflow-hidden p-1">Registered Businesses</h2>
          <div className="flex space-x-4">
            {/* Category Selection */}
            <select
              className="border-purple-800 border-2 cursor-pointer  bg-black  rounded-md p-2"
        ...
        <div className="  p-5 rounded-lg bg-slate-800 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {filteredShops.map(shop => (
            <Card key={shop._id} className="overflow-hidden p-3  text-slate-200 bg-black/50" >
              <div className="relative h-48">
                ...
                <div className="absolute bottom-4 left-4 bg-white rounded-full p-1 shadow-md">
                  <img
                    src={shop.logo}
                    alt={`logo`}
                    className="w-16 h-16 rounded-full text-slate-900"
                  />
                </div>
              </div>
              <CardHeader>
              ...
              <CardFooter>
                <Button className="bg-purple-500/50 border-slate-400 border-2  " onClick={()=>goToShop(shop._id)}>View Shop</Button>
              </CardFooter>
            </Card>
          ))}
          {filteredShops.map(shop => (
